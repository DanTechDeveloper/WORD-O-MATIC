...
export default function StudentProfile({ minimal = false }) {
    ...
    return (
        <>
            <header
                className={`fixed top-0 right-0 ${minimal ? "left-0" : "left-0 lg:left-64"} h-16 lg:h-20 flex items-center justify-between px-4 lg:px-8 z-40 bg-slate-950 border-b-4 border-purple-900 shadow-[0_4px_0_0_#1e1b4b]`}
            >
                ...
                <div className="flex items-center gap-4 shrink-0">
                    <Link
                        ...
                        method="post"
                        as="button"
                        className="flex items-center gap-2 bg-pink-600 text-white px-3 lg:px-4 py-2 rounded-xl border-b-4 border-pink-900 active:translate-y-1 active:border-b-0 transition-all font-black text-xs uppercase"
                    >
