...
export default function StudentProfile({ disableNav }) {
    const { auth } = usePage().props;
    const avatar  = auth.user.student.avatar;
    return (
        <header className={`fixed top-0 left-0 right-0 h-16 lg:h-20 flex items-center justify-between px-4 lg:px-8 z-40 bg-background border-b-2 border-outline ${disableNav ? "pointer-events-none opacity-60" : ""}`}>
