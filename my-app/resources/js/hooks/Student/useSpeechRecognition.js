...
export function useSpeechRecognition({
    ...
    const sentenceSpeechStoppedAtRef = useRef(0);
    ...
    useEffect(() => {
        ...
        speechStoppedAtRef.current = 0;
        mispronouncedInThisWordRef.current = false;
        sentenceSpeechStoppedAtRef.current = 0;
        clearTimers(mispronounceTimeoutRef, restartTimerRef, sentenceTimeoutRef);
        gracePeriodEndRef.current = Date.now() + 500;
        if (wasMatched && recognitionRef.current && propsRef.current.isActive) {
            try { recognitionRef.current.stop(); } catch {}
        }
        if (propsRef.current.isActive && recognitionRef.current && !isRecognitionStartedRef.current) {
            try { recognitionRef.current.start(); isRecognitionStartedRef.current = true; } catch {}
        }
    }, [targetWord]);
