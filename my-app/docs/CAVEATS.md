...
| BF12 | Echo feedback re-recognition (Word Blast) | The program's own success ("Great!")/mispronounce ("Try Again!") audio could be heard by the open mic and fuzzy-match the next short target word in Word Blast, skipping it. | Mode-aware `isActive` gate: Word Blast closes mic for the 500/800ms feedback windows; Story Quest keeps mic live continuously (full-sentence match not fooled, stopping would clip user's continuous read). | `GameplayReadMode.jsx:90` (mic gate), `GameplaySpeakMode.jsx:95` (no gate) |
