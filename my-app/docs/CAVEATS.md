...
| BF6 | Deadline Cutoff & Excel Charts | ISO date string (`YYYY-MM-DDTHH:MM`) passed directly to SQL `created_at <= cutoff` compared space vs `'T'` (ASCII 32 < 84), selecting all rows on current day regardless of cutoff time. Validation `after:now` blocked setting deadline today. Excel export lacked charts & had namespace typos. | Cutoff timestamp parsed & formatted as `Y-m-d H:i:s` in `WordModule` and `ParagraphModule`. Validation updated to `after_or_equal:today`. Excel export implements `WithCharts` with native Pie Chart and Bar Chart. | `WordModule::trainingWordsForUsers`, `ParagraphModule::trainingWordsForUsers`, `TeacherController::saveDeadline`, `ClassReportSheet`, `ReportTest`. |
