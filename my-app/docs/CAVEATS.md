...
| BF7 | Post-deadline drift (Option A) | After the deadline, students could keep replaying; the emailed accuracy (`wordBlastAcc`/`storyQuestAcc`) is the live denormalized value, so late retries shifted it past the as-of-deadline training-word cutoff (old H5). | Gameplay is fully blocked post-deadline: `finishRound()` skips progress (server POST), while GET endpoints (`gameplayReadMode`, `gameplaySpeakMode`) redirect to levels. Tutorial modules remain allowed (students mid-onboarding). Frontend: PLAY AGAIN disabled, completed level cards non-clickable, PLAY/CONTINUE non-clickable. No deadline set → gameplay fully open. | `GameplayTest::test_round_logs_session_but_skips_progress_when_deadline_passed`, `test_gameplay_page_redirects_when_deadline_passed`, `test_speak_gameplay_page_redirects_when_deadline_passed`, `test_read_tutorial_module_still_loads_when_deadline_passed`, `test_speak_tutorial_module_still_loads_when_deadline_passed`. |
